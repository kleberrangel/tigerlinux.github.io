...
mysql -u zabbixuser -h 172.16.10.221 -pP@ssw0rd zabbixdb < /workdir/gunzip.sql
```
>
...
Now, edit the file `/etc/zabbix/zabbix_server.conf` in both nodes. 

Setup the data base access, changing the following lines:
...
**Remember: The IP 172.16.10.221 is the MariaDB Cluster LBaaS VIP.**

Save the changes in both servers.

In the primary server (172.16.10.57), restart zabbix using the control script:

```
/usr/local/bin/zabbix-ha-control.sh stop
/usr/local/bin/zabbix-ha-control.sh start
```
